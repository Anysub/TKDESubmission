...
class IdentityPrep(nn.Module):
    def __init__(self, input_dim, n_nodes=None, embedding_dim=64,pre_trained=None):
...
class NodeEmbeddingPrep(nn.Module):
    def __init__(self, input_dim, n_nodes, pre_trained=None, embedding_dim=64):
        ...
        if pre_trained is not None:
            ...
            self.embedding.from_pretrained(pre_trained, padding_idx=-1,freeze=False)
...
class LinearPrep(nn.Module):
    def __init__(self, input_dim, n_nodes, embedding_dim=64,pre_trained=None):
        """ adds node embedding """
...
class AttentionAggregator3(nn.Module):
    def __init__(self, input_dim, output_dim, edge_dim, activation, hidden_dim=512,
        ...
        self.att2 = nn.Sequential(*[
            nn.Linear(input_dim+edge_dim, 1, bias=False),
            # nn.Tanh(),
            # nn.Linear(hidden_dim, hidden_dim, bias=False),
        ])
    ...
    def forward(self, x, neibs, edge_emb, mask):
        ...
        neib_att = self.att2(neibs).view(x.size(0),-1)
        x_att = self.att(x).view(-1,1)
...
class AttentionAggregator5(nn.Module):
    def __init__(self, input_dim, output_dim, edge_dim, activation, hidden_dim=512,
                 dropout=0.5,attn_dropout=0,
                 concat_node=True, concat_edge=True, batchnorm=False):
        super(AttentionAggregator5, self).__init__()
        self.attn_dropout = nn.Dropout(p=attn_dropout)
        self.att = nn.Sequential(*[
            nn.Linear(input_dim, hidden_dim, bias=False),
            nn.Tanh(),
    ...
    def forward(self, x, neibs, edge_emb, mask):
        ...
        edge_emb = self.edge_fc(edge_emb)
...
aggregator_lookup = {
    ...
    'attention5':AttentionAggregator5,
