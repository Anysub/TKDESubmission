...
class NodeProblem(object):
    def __init__(self, problem_path, problem, schemes, device,train_per, K=10, input_edge_dims = 128,emb_len=32):
        ...
        _, self.homo_feat = read_homograph(path=problem_path,problem=problem,emb_len=emb_len)
        # self.homo_feat = None
        if self.feats is not None:
            # self.feats = np.hstack([self.feats, self.homo_feat[:self.n_nodes]])
            pass
        else:
            # self.feats = self.homo_feat[:self.n_nodes]
            pass
